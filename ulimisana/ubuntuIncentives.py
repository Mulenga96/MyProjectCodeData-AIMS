...
def initialise_ubuntuIncentives(NoOfFamilies,popSize,ageAverage,ageStdev,dim,lb,ub,time_iter):
    ...
    Parameter:
    -----------
        NoOfFamilies     : The preferred number of families
        popSize          : The preferred population size
        ageAverage       : Average age of individuals in each family
    ...
    ---------
        ...
        fam_payoffs     : This is an empty dataframe which will contain the updated agent's family payoffs over time
        comm_payoffs    : This is an empty dataframe which will contain the updated agent's community payoffs over time
    """
...
def ubuntuIncentives(objFunction,df,ind_position,ind_payoffs,fam_payoffs, 
    ...
    Parameters:
    ----------------
            objFunction             : This is the objective function to be evaluated. 
            ...
            famMinPayoffThreshold   : This is s scaler that ranges [0,1] which determines the factor by which the poor                                               family's payoffs should be lower than. 
                                      This is multiplied to the average payoffs of individuals in the family.
            commMinPayoffThreshold  : This is a scaler that ranges [0,1] which determines the factor by which the poor family's                                       payoffs should be lower than. 
                                      This is multiplied with the average payoffs of individuals in the community
            phi + rho = 1           : These are used as weighted average where the direction of each is determined by                                                 whether the minimum family or community payoff was met.
    ...
    #print(f'IndividualPosition:{IndividualPosition}')
    iter_idx = 'iter_'+str(time_iter)
    IndividualPayoff =  -1*objFunction(x)
    print(f'IndividualPayoff:{IndividualPayoff}')
    print(f'IndividualPayoff:{type(IndividualPayoff)}')
    ...
    for i in famID:
       ...
       # print(f'fam_{i}:{fam}')
        ...
        dependentsPop = fam[c]
        providersPop = fam[~c]
        dependentsPayoff = sum(dependentsPop['IndPayoff'])  # Payoffs of dependents.
        providersPayoff = sum(providersPop['IndPayoff'])
        df2['Dependents'][i] = len(dependentsPop)
        df2['Providers'][i]= len(providersPop)
        #print(f'Dependents:{dependentsPayoff}, MeanFam:{min_FamilyPayoff}')    
        if dependentsPayoff > min_FamilyPayoff:
            rho = phi
        elif dependentsPayoff <= min_FamilyPayoff:
            rho = 1 - phi
        familyPayoff = rho*providersPayoff + (1 - rho)*dependentsPayoff  # Family payoffs.
    ...
    if np.mean(poorPayoff) > min_communityPayoff:
        ...
        rho = 1 - phi
    communityPayoff = rho*wealthyPayoff + (1-rho)*poorPayoff   #  Communiyty payoffs.
    comm_payoffs.loc[iter_idx] = communityPayoff    

    # Payoff Updates
    
    # Community payoff updates
    if time_iter ==0:         
        commPayoffDelta = 0
