...
def initialise_position(popSize,dim,time_iter):
    ...
    Each position variable is given a default name of "IndPosition' + str(dim). eg IndPosition2 indicates the third variable of the dimensions of the objective function dimensions. [i.e Prefix = 'IndPosition]
...
def myfamGlocalBest(df,fam_payoffs,ind_payoffs,ind_position,myFamID):
    ...
    -----------
        ...
        fam_payoffs: This indicates a family's payoffs dataframe
        ind_payoffs : Individual Payoffs DataFrame
        ind_position : Individual's positions
...
def famGlobalBest(df,current_iteration,previous_iteration,fam_payoffs,ind_payoffs,ind_position):
    ...
    This function calculates the best family's best agent's position
    
    Parameters:
    -----------
        df : This is a dataframe with the information about the agents. Includes their family, position, age and payoff details.
        current_iteration : This indicates the current iteration that the whole solution search is in.
        previous_iteration : This indiacate the previous iteration that the whole solution search was in. 
        fam_payoffs: This indicates the family's payoffs dataframe
        ind_payoffs : Individual Payoffs DataFrame
        ind_position : Individual's positions
    ...
    iteration   : The iteration at which an agent's family was at its best performance. 
    """
...
def commGlobalBest(df,comm_payoffs,fam_payoffs,ind_payoffs,ind_position):
    """
    This function calculates the best agents from the best performing family when the community was at best performance
    
    Parameters:
    -----------
        df : This is a dataframe with the information about the agents. Includes their family, position, age and payoff details.
        comm_payoffs : This is a community payoff's Dataframe
        fam_payoffs: This indicates the family's payoffs Dataframe
        ind_payoffs : Individual Payoffs Dataframe
        ind_position : Individual's positions
    ...
    bestComFamID : ID of the Best Global Family in the community's global performance.
    iteration   : The iteration at which an agent's family was at its best performance. 
    """
